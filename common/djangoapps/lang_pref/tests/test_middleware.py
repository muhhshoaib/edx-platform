...
class TestUserPreferenceMiddleware(TestCase):
    ...
    def test_user_preferences_with_supported_browser_language(self):
        """
        test: user preferred language has been removed from system so browser language should be set in user session
        and in preferences if it is supported by system for authenticated users.
        """
        set_user_preference(self.user, LANGUAGE_KEY, 'eo')
        self.middleware.process_request(self.request)
        self.assertEqual(self.request.session.get(LANGUAGE_SESSION_KEY), 'ar')   # pylint: disable=no-member
        self.assertEqual(get_user_preference(self.request.user, LANGUAGE_KEY), 'ar')

    @mock.patch('lang_pref.middleware.released_languages', mock.Mock(return_value=[('en', 'english')]))
    def test_user_preferences_with_system_default_language(self):
        """
        test: user preferred language has been removed from system so system default language should be set in user
        session and in preferences if browser language is not supported by system for authenticated users.
        """
        system_language = settings.LANGUAGE_CODE
        set_user_preference(self.user, LANGUAGE_KEY, 'eo')
        self.middleware.process_request(self.request)
        self.assertEqual(self.request.session.get(LANGUAGE_SESSION_KEY), system_language)   # pylint: disable=no-member
        self.assertEqual(get_user_preference(self.request.user, LANGUAGE_KEY), system_language)
