...
class SelfPacedDateOverrideTest(ModuleStoreTestCase):
    ...
    def test_course_access_to_beta_users(self):
        ...
        sp_course, sp_section = self.setup_course(**course_options)
        beta_tester = BetaTesterFactory(course_key=sp_course.id)

        # Verify course is `self_paced` and course has start start but not section.
        self.assertTrue(sp_course.self_paced, "Course is self_paced")
        self.assertEqual(sp_course.start, after_month, "Course has start date")
        self.assertIsNone(sp_section.start, "Section start date is None")

        # Verify tht non-staff user do not have access to the course
        self.assertFalse(has_access(self.non_staff_user, 'load', sp_course))

        # Verify beta tester can access the course as well as the course sections
        self.assertTrue(has_access(beta_tester, 'load', sp_course))
        self.assertTrue(has_access(beta_tester, 'load', sp_section, sp_course.id))
